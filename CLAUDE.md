...
**Data flow:** `RSSAppApp` (creates `ModelContainer` + runs migration) → `ContentView` (creates `SwiftDataFeedPersistenceService` from `modelContext`) → `HomeView` (`NavigationStack` root with `HomeGroup` rows: All Articles, Unread Articles, Saved Articles, user-created feed groups, All Feeds) → All Articles/Unread Articles/Saved Articles/Feed Groups (cross-feed paginated lists) / All Feeds → `FeedListView` → `ArticleListView` (`FeedViewModel`, cache-first: show cached articles immediately, fetch + upsert in background) → `ArticleReaderView` → `ArticleSummaryView` → `ArticleDiscussionView`. Persistence is SwiftData via `FeedPersisting` (`PersistentFeed` → `PersistentArticle` → `PersistentArticleContent`, `PersistentFeedGroup` → `PersistentFeed`), with transient parser structs bridged via `ModelConversion`. All view models are `@Observable @MainActor`; services are protocol-abstracted and `Sendable`.
