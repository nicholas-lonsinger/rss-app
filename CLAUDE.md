...
**Data flow:** `RSSAppApp` (creates `ModelContainer` + runs migration) → `ContentView` (creates `SwiftDataFeedPersistenceService` from `modelContext`) → `HomeView` (`NavigationStack` root with four `HomeGroup` rows: All Articles, Unread Articles, Saved Articles, All Feeds) → All Articles/Unread Articles/Saved Articles (cross-feed paginated lists) / All Feeds → `FeedListView` → `ArticleListView` (`FeedViewModel`, cache-first: show cached articles immediately, fetch + upsert in background) → `ArticleReaderView` → `ArticleSummaryView` → `ArticleDiscussionView`. Persistence is SwiftData via `FeedPersisting` (`PersistentFeed` → `PersistentArticle` → `PersistentArticleContent`), with transient parser structs bridged via `ModelConversion`. All view models are `@Observable @MainActor`; services are protocol-abstracted and `Sendable`.
...
- Create mock implementations using protocols (see `RSSAppTests/Mocks/`)
...
- Pin "invariants" that would only change if someone deliberately rewrote the function (e.g. "DateFormatter is not mutated post-init")
- Re-verify behavior already covered by an existing test at a different layer

**Test seams in production code are a smell.** If a test requires adding an injection point, mock hook, or alternative code path to `RSSApp/` *solely* so the test can reach a branch, the right answer is usually to drop the test, not add the seam. Seams are justified only when the same seam has a non-test use (e.g. dependency injection that also enables previews or alternate backends).
