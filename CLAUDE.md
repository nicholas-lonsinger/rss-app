...
**Data flow:** `RSSAppApp` (creates `ModelContainer` + runs migration) → `ContentView` (creates `SwiftDataFeedPersistenceService` from `modelContext`) → `HomeView` (`NavigationStack` root with three `HomeGroup` rows: All Articles, Unread Articles, All Feeds) → All Articles/Unread Articles → flat chronological cross-feed article list → tap article → `ArticleReaderView` | All Feeds → `FeedListView` (`NavigationStack`, `FeedListViewModel` + `FeedPersisting` + `OPMLService` + `FeedFetchingService` + `FeedIconService`) → tap feed → `ArticleListView` (`FeedViewModel` per feed, cache-first loading) → tap article → `ArticleReaderView`. `FeedFetchingService` fetches feed XML via `URLSession`, `RSSParsingService` parses both RSS 2.0 and Atom feeds with `XMLParser` into `Article` structs (transient), which are upserted into SwiftData as `PersistentArticle` records. Feeds, articles, and extracted content are persisted in SwiftData via `FeedPersisting` (`PersistentFeed` → `PersistentArticle` → `PersistentArticleContent`). `FeedIconService` resolves and caches feed favicons/logos to disk (feed XML → site HTML → /favicon.ico fallback), displayed via `FeedIconView` in the feed list. `ArticleThumbnailService` downloads article thumbnail images (direct URL → `og:image` fallback), resizes to 120×120px, and caches as JPEG to disk; `ArticleThumbnailView` resolves and loads them on-demand off the main thread. Articles support read/unread tracking and offline browsing. Read/unread state on `PersistentArticle.isRead` is the single source of truth, reflected across all views. OPML import/export via `OPMLService` uses `SubscribedFeed` conversion at the boundary. Pull-to-refresh re-fetches feed metadata, upserts articles, and resolves icons for all feeds.
...
**Guidelines:**
- Every new service or view model should declare its own `private static let logger = Logger(category: "ComponentName")`
