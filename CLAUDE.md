...
**Data flow:** `RSSAppApp` (creates `ModelContainer` + runs migration) → `ContentView` (creates `SwiftDataFeedPersistenceService` from `modelContext`) → `HomeView` (`NavigationStack` root with built-in `HomeGroup` rows: All Articles, Unread Articles, Saved Articles, All Feeds; plus dynamic user-created groups section) → All Articles/Unread Articles/Saved Articles (cross-feed paginated lists) / User Groups (`GroupArticleSource`, per-group paginated lists) / All Feeds → `FeedListView` → `ArticleListView` (`FeedViewModel`, cache-first: show cached articles immediately, fetch + upsert in background) → `ArticleReaderView` → `ArticleSummaryView` → `ArticleDiscussionView`. Persistence is SwiftData via `FeedPersisting` (`PersistentFeed` → `PersistentArticle` → `PersistentArticleContent`; `PersistentFeedGroup` ↔ `PersistentFeedGroupMembership` ↔ `PersistentFeed` for many-to-many group membership), with transient parser structs bridged via `ModelConversion`. All view models are `@Observable @MainActor`; services are protocol-abstracted and `Sendable`.
