...
**Data flow:** `RSSAppApp` (creates `ModelContainer` + runs migration) → `ContentView` (creates `SwiftDataFeedPersistenceService` from `modelContext`) → `FeedListView` (`NavigationStack` root, `FeedListViewModel` + `FeedPersisting` + `OPMLService` + `FeedFetchingService`) → tap feed → `ArticleListView` (`FeedViewModel` per feed, cache-first loading) → tap article → `ArticleReaderView`. `FeedFetchingService` fetches feed XML via `URLSession`, `RSSParsingService` parses both RSS 2.0 and Atom feeds with `XMLParser` into `Article` structs (transient), which are upserted into SwiftData as `PersistentArticle` records. Feeds, articles, and extracted content are persisted in SwiftData via `FeedPersisting` (`PersistentFeed` → `PersistentArticle` → `PersistentArticleContent`). Articles support read/unread tracking and offline browsing. OPML import/export via `OPMLService` uses `SubscribedFeed` conversion at the boundary. Pull-to-refresh re-fetches feed metadata and upserts articles for all feeds.
