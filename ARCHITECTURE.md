...
├── Models/                             # Data models
│   ├── AppBadgeMode.swift              # Enum — badge display modes (count, indicator, off) with CaseIterable, Identifiable, Sendable; persisted via UserDefaults
...
│   ├── IdentifiableIndex.swift         # Lightweight Identifiable wrapper for Int indices — enables fullScreenCover(item:) with index-based navigation; includes Binding<Int?> extension with shared identifiableIndex/nonOptionalIndex helpers
...
│   ├── AppBadgeService.swift            # AppBadgeUpdating protocol + badge mode dispatch (count/indicator/off) via UNUserNotificationCenter with badge-only permission request
...
│   ├── SettingsView.swift              # Top-level settings page with NavigationLink rows pushing App Badge, API Key, Article Limit, and Import/Export sub-screens
...
├── Mocks/
│   ├── MockAppBadgeService.swift            # AppBadgeUpdating mock with call count tracking and injectable mode
...
├── Models/
│   ├── AppBadgeModeTests.swift         # AppBadgeMode enum cases, display labels, rawValue round-trips, default mode
...
│   ├── IdentifiableIndexBindingTests.swift # Binding<Int?> extension: identifiableIndex/nonOptionalIndex get/set, roundtrip, combined usage
...
**Article reader navigation.** `ArticleReaderView` receives the originating list's article array, a `Binding<Int>` to the current index, and an optional `loadMore` closure returning `LoadMoreResult`. Bottom toolbar displays previous (chevron.backward) and next (chevron.forward) buttons; previous is disabled at index 0, next is disabled at the last loaded article when no more pages exist. When the user navigates past the last loaded article, the `loadMore` closure triggers pagination (`loadMoreAndReport()` / `loadMore*ArticlesAndReport()`); on `.loaded` the reader advances to the next article, on `.exhausted` it stays put, and on `.failed` it surfaces the error message via an alert. On navigation, `ReaderExtractionState` is reset, the `ArticleReaderWebView` reloads via `.id(article.articleID)`, and the new article is marked as read. List views pass their article arrays and use `IdentifiableIndex` (an `Identifiable` wrapper for `Int`) to bridge the index-based model with `fullScreenCover(item:)`. The `Binding<Int?>` extension in `IdentifiableIndex.swift` provides shared `identifiableIndex` and `nonOptionalIndex` computed properties used by all four list views: `ArticleListView`, `AllArticlesView`, `UnreadArticlesView`, and `SavedArticlesView`.
...
| App icon badge via `UNUserNotificationCenter.setBadgeCount` | Requests badge-only notification permission (no alerts/sounds); three user-selectable modes (count, indicator, off) persisted in UserDefaults; updated as fire-and-forget `Task` from `HomeViewModel.loadUnreadCount()` at natural sync points (refresh, mark-read, app foreground) |
| Concrete `AppBadgeService` in `AppBadgeView` | Existential `any AppBadgeUpdating` property setters require mutable access, incompatible with SwiftUI's immutable view structs; protocol abstraction is used in `HomeViewModel` for testing |

## Test Coverage

**54 test files: 35 test suites, 14 mock implementations, 4 shared helpers, 1 HTML fixture.**
