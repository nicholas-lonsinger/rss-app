...
**Total: 67 source files + 1 resource, 51 test source files + 1 fixture.**
...
| Offset-based pagination (page size 50) | Simple `fetchOffset`/`fetchLimit` on `FetchDescriptor`; deduplication filter on append prevents duplicates from dataset shifts; `hasMore` flag preserved on error so user can retry by tapping next again (errors are transient and surfaced via `LoadMoreResult.failed` alert); previous list preserved on reload failure |
...
**51 test files: 33 test suites, 13 mock implementations, 4 shared helpers, 1 HTML fixture.**
