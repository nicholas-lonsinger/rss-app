...
**Update detection plumbing (issue #74).** The RSS parser captures Atom `<updated>`, `dc:modified`, `dcterms:modified`, and `atom:updated` as a first-class signal in `Article.updatedDate`, distinct from `publishedDate`. `dc:date` and `dcterms:created` are accepted as Dublin Core publication-date fallbacks but never as update signals. `PersistentArticle` carries the value through (`updatedDate: Date?`) plus a reserved `wasUpdated: Bool` flag for the upsert path to set when a publisher revision is detected, and exposes a non-persisted `displayedPublishedDate` computed property that clamps `publishedDate` against `fetchedDate` so list rows can show a stable original-publication time even after a follow-up to issue #74 starts mutating `sortDate` on update detection. **PR 1 is data plumbing only**: `FeedPersistenceService.upsertArticles` is still insert-only and no production view consumes the new fields. Detection logic and UI surface are deferred to follow-up PRs.
