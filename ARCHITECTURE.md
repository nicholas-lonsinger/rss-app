...
│   ├── ArticleRowView.swift            # Single article row — thumbnail, title, snippet, read/unread styling. Hosts `ArticleRowDateLine` (shared with `CrossFeedArticleRowView`) which displays the original publication time, an optional "Updated [date]" suffix when the publisher has revised the article, and an orange "Updated" capsule badge when `wasUpdated == true` (issue #74)
