...
`KeychainServicing` is a `Sendable` protocol. `KeychainService` wraps `Security` framework (`kSecClassGenericPassword`) to save, load, and delete the Anthropic API key. The key is stored encrypted by the OS and never touches any file accessible to git. A protocol extension on `KeychainServicing` provides API key convenience methods (`hasAPIKey`, `loadAPIKey()`, `saveAPIKey(_:)`, `deleteAPIKey()`) that centralize the `"anthropic-api-key"` account identifier, eliminating duplication across views and view models.
...
| KeychainService | KeychainServiceTests.swift | Save/load roundtrip, load when empty, delete clears value, overwrite updates value, API key convenience extension (hasAPIKey, loadAPIKey, saveAPIKey, deleteAPIKey roundtrips, apiKeyAccount constant) |
