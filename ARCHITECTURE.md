...
| `returningFromReader` flag suppresses post-pop reload | All four article list views (`AllArticlesView`, `UnreadArticlesView`, `SavedArticlesView`, `ArticleListView`) hold a `@State private var returningFromReader = false` that is armed in the row tap action immediately before assigning `selectedArticleIndex` (which pushes the reader) and consumed by the existing `hasAppeared`-guarded `onAppear` to skip exactly one reload. Without this flag, popping the reader re-fires the parent's `onAppear`, which would re-query persistence and (a) drop articles the user just marked as read in the reader from the unread list / per-feed "Show Unread Only" view, and (b) reset pagination depth and scroll position on the all/saved lists. The snapshot is still refreshed on explicit triggers — pull-to-refresh, tab change, sort/filter toggle, mark all as read — so it never goes stale indefinitely |
