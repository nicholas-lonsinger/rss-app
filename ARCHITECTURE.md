...
| `sortDate` vs `publishedDate` on `PersistentArticle` | `publishedDate` stores the publisher-supplied value verbatim (preserved for a planned content-update detection feature that compares pubDate values across refreshes). `sortDate` is a separate non-optional field computed at insert as `min(publishedDate ?? now, now)` — clamping any future date to ingestion time. All sort/retention/display paths use `sortDate`. This decouples sort order from publisher data so scheduled posts (e.g., the Cloudflare blog's upcoming-content feed, whose `pubDate` lies hours ahead of `lastBuildDate`) don't pin to the top of newest-first lists or get evicted prematurely from retention. `sortDate` is computed once at insert and never updated, so an article's sort position is stable over its lifetime. |
