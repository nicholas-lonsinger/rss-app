...
**Total: 70 source files + 1 resource, 58 test source files + 1 fixture.**
...
| Reader-internal `currentIndex` state | `ArticleReaderView` seeds `@State private var currentIndex: Int` from the parent's `initialIndex` parameter rather than holding a `Binding` to the parent's `Int?`. The previous design (`Binding<Int>` derived from `Binding<Int?>` via a `nonOptionalIndex` helper) crashed during pop-animation teardown: `navigationDestination(isPresented:)` would clear the parent's optional to `nil` while SwiftUI was still re-evaluating the departing reader's body, causing the derived non-optional binding to read `nil`. Owning the index inside the reader severs that dependency entirely |
